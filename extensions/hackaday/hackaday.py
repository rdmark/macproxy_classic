...
def process_html(content, url):
    ...
    if comments_div:
        for p in comments_div.find_all("p"):
            if (
                "Please be kind and respectful" in p.get_text()
                or "This site uses Akismet" in p.get_text()
            ):
                p.decompose()
    ...
    entry_content_divs = soup.find_all(
        "div", class_="entry-content", itemprop="articleBody"
    )
    ...
    if body_tag:
        ...
        search_input = soup.new_tag(
            "input",
            **{"type": "text", "size": "49", "required": True, "autocomplete": "off"},
        )
        search_input["name"] = "s"
...
"""
    ...
    if (
        "hackaday.com/blog/" in url
        or "hackaday.com/author/" in url
        or "hackaday.com/page/" in url
    ):
        ...
        for article in articles:
            ...
            if entry_meta:
                ...
                if author_link:
                    ...
                    new_meta = (
                        f'By <a href="{author_url}">{author_name}</a> | {date}<br><br>'
                    )
    ...
    if nav_links:
        ...
        older_html = (
            f'<a href="{older_link_div.a["href"]}">Older posts</a>'
            if older_link_div
            else ""
        )
        newer_html = (
            f'<a href="{newer_link_div.a["href"]}">Newer posts</a>'
            if newer_link_div
            else ""
        )
    ...
    if "/blog/" in url and "s=" in url:
        ...
        if header:
            ...
            if title_b:
                article_title = title_b.text.strip().split("<br>")[
                    0
    ...
    hackaday_native_search = soup.find(
        "form",
        attrs={"action": "https://hackaday.com/", "method": "get", "role": "search"},
    )
