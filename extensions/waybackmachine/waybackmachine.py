        ...
        if response.status_code == 200:
            ...
            if len(data) > 1:  # First row is header
                ...
                snapshots.sort(
                    key=lambda x: (
                        abs(
                            int(x[1]) - target_timestamp
                        ),  # Primary sort: absolute distance from target
                        -int(
                            x[1]
                        ),  # Secondary sort: reverse timestamp (prefer earlier dates)
                    )
                )
...
def make_archive_request(url, follow_redirects=True, original_timestamp=None):
    ...
    try:
        ...
        timestamp_to_use = (
            original_timestamp if original_timestamp else find_closest_snapshot(url)
        )
        ...
        if response.status_code == 200 and follow_redirects:
            ...
            if "window.location.replace" in content:
                redirect_match = re.search(
                    r'window\.location\.replace\(["\'](.+?)["\']\)', content
                )
                if redirect_match:
                    redirect_url = redirect_match.group(1).strip()
...
def extract_original_url(url, base_url):
    ...
    try:
        ...
        base_match = re.search(
            r"/web/\d{14}(?:im_|js_|cs_|fw_|oe_)?/(?:https?://)?([^/]+)/?", base_url
        )
        ...
        if re.search(timestamp_pattern, url):
            match = re.search(
                r"/web/\d{14}(?:im_|js_|cs_|fw_|oe_)?/(?:https?://)?(.+)", url
            )
            if match:
...
def handle_request(req):
    ...
    if is_wayback_domain:
        if req.method == "POST":
            ...
            if action == "enable":
                ...
                _, last_day = calendar.monthrange(
                    selected_year, months.index(selected_month) + 1
                )
                if selected_day > last_day:
                    selected_day = last_day

                selected_date = datetime.datetime(
                    selected_year, months.index(selected_month) + 1, selected_day
                )
                current_date = datetime.datetime.now()
                ...
                date_update_message = (
                    f"{selected_month} {selected_day}, {selected_year}"
                )
        ...
        return (
            render_template_string(
                ...
                date_update_message=date_update_message,
            ),
            200,
        )
    ...
    try:
        ...
        return (
            f"<html><body><p>Error fetching archived page: {str(e)}</p></body></html>",
            500,
            {"Content-Type": "text/html"},
        )
